...
/// </summary>
public class RegionGenerator : IGraphPipelineStage
{
    ...
    public void Initialize(WorldSettings settings)
    {
        ...
        _prng = new System.Random(settings.WorldSeed + (int)WorldSeedChannel.RegionGenerator);  // 201
    }
        ...
        CancellationToken ct)
    {
        try
        {
            ...
            _regionResult.UpdateNodeIndices(); // 변환 후 인덱스 갱신
    ...
    private (List<Node> cluster, List<NodeConnection> clusterConnections) GenerateRoomsInRegion(Node regionNode)
    {
        ...
        // 2. 방 확장 루프
        if(regionData.DefaultMinCount <= 0 || regionData.DefaultMaxCount <= 0 || regionData.DefaultMinCount > regionData.DefaultMaxCount)
        {
            Debug.LogWarning($"[RegionGen] Region '{regionData.RegionName}'의 DefaultMinCount 또는 DefaultMaxCount가 잘못되었습니다.");
        }
        int defaultRoomCount = _prng.Next(regionData.DefaultMinCount, regionData.DefaultMaxCount + 1) + regionData.EssentialRooms.Count;
    ...
    private Node PickParentRoom(List<Node> placedRooms, RegionBranch branch)
    {
        switch (branch)
        {
            ...
            case RegionBranch.Most: // 성게 (Hub / Star)
                ...
                return placedRooms.OrderBy(n => n.RoomDepth).ThenBy(n => _prng.NextDouble()).First();

            case RegionBranch.Default: // 랜덤 (Tree)
            default:
                // 아무거나 랜덤 선택 
                return placedRooms[_prng.Next(0, placedRooms.Count)];
        }
    }
    ...
    private void AssignEssentialRooms(List<Node> clusters, RegionData regionData)
    {
        ...
        foreach (EssentialRoomEntry essential in regionData.EssentialRooms)
        {
            ...
            Node targetNode = (candidates.Count > 0) ? candidates[_prng.Next(0, candidates.Count)] :
        ...
        foreach (List<Node> cluster in _regionToCluster.Values)
        {
            ...
            for (int i = 0; i < cluster.Count; i++)
            {
                for (int j = i + 1; j < cluster.Count; j++)
                {
                    ...
                    // 최종 : 확률 체크
                    if (_prng.NextDouble() < loopChance)
                    {
