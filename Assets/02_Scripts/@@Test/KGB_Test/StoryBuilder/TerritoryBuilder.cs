...
public class TerritoryBuilder
{
    ...
    private const float DEFAULT_SEPARATION_GAP = 2.0f;
    ...
    private WorldMapData _worldMapData; // ★ 데이터 바구니 추가
    ...
    /// 맵 분할 메인 파이프라인
    /// </summary>
    public async UniTask<WorldMapData> TerritoryBuildAsync(
        GraphResult result,
    ...
    private void InitializeSpatialGrid()
    {
        ...
        _nodeSpatialGrid = new SpatialGrid<NodeSpatialData>(
            _worldMapData.TileGridSize.x,
            _worldMapData.TileGridSize.y,    
            cellSize
        );
    ...
    private async UniTask FitNodesToTileGridAsync()
    {
        ...
        float targetWidth = _worldMapData.TileGridSize.x * (1f - DEFAULT_PADDING * 2);
        float targetHeight = _worldMapData.TileGridSize.y * (1f - DEFAULT_PADDING * 2);
        ...
        Vector2 targetCenter = new Vector2(_worldMapData.TileGridSize.x * 0.5f, _worldMapData.TileGridSize.y * 0.5f);
    ...
    private void ValidateNodePositions(List<Node> nodes)
    {
        ...
        foreach (var node in nodes)
        {
            if (node.Position.x < 0 || node.Position.x > _worldMapData.TileGridSize.x ||
                node.Position.y < 0 || node.Position.y > _worldMapData.TileGridSize.y)
            {
                Debug.LogError($"🚨 노드가 맵 범위를 벗어남: {node.Position} (맵 크기: {_worldMapData.TileGridSize})");
                hasInvalidNode = true;
            }
        }
    ...
    private async UniTask GenerateNoiseWorldAsync()
    {
        ...
        float offsetX = safeSeed * 100f, offsetY = safeSeed * 100f;
        ...
        for (int x = 0; x < _worldMapData.TileGridSize.x; x++)
        {
            for (int y = 0; y < _worldMapData.TileGridSize.y; y++)
            {
                ...
                _worldMapData.NoiseWorld[x, y] = (noise / maxValue) * _partiSettings.noiseStrength;
    ...
    private async UniTask AssignTerritoriesAsync()
    {
        ...
        for (int x = 0; x < _worldMapData.TileGridSize.x; x++)
        {
            for (int y = 0; y < _worldMapData.TileGridSize.y; y++)
            {
                ...
                // ★ [최적화] SpatialGrid를 사용한 최근접 노드 검색
                ...
                _worldMapData.TerritoryWorld[x, y] = finalOwner;
        ...
        Vector2 tilePos, int x, int y)
    {
        ...
        // 노이즈 오프셋 가져오기
        float noiseOffset = _worldMapData.NoiseWorld[x, y];
